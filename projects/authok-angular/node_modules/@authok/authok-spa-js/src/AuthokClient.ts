 ...
 * @ignore
 */
const GET_TOKEN_SILENTLY_LOCK_KEY = 'authok.lock.getTokenSilently';

/**
 * @ignore
 */
const buildOrganizationHintCookieName = (clientId: string) =>
  `authok.${clientId}.organization_hint`;

/**
 * @ignore
 */
const OLD_IS_AUTHENTICATED_COOKIE_NAME = 'authok.is.authenticated';

/**
 * @ignore
 */
const buildIsAuthenticatedCookieName = (clientId: string) =>
  `authok.${clientId}.is.authenticated`;
 ...
 */
export default class AuthokClient {
  ...
  constructor(private options: AuthokClientOptions) {
  ...
  private _url(path: string) {
    const authokClient = encodeURIComponent(
      btoa(JSON.stringify(this.options.authokClient || DEFAULT_AUTHOK_CLIENT))
    );
    return `${this.domainUrl}${path}&authokClient=${authokClient}`;
  }
