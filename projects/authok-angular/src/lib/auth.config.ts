...
export interface AuthConfig {
  /**
   * Your Authok account domain such as `'example.cn.authok.cn'`,
   * `'example.cn.authok.cn'` or , `'example.mycompany.com'`
   * (when using [custom domains](https://docs.authok.cn/docs/custom-domains))
   */
  domain: string;
   ...
   * "Allowed Callback URLs" field in your Authok Application's
   ...
   * an Authok application and attempt to exchange the code for a token.
  ...
  /**
   * If true, refresh tokens are used to fetch new access tokens from the Authok server.
...
/**
 * Gets and sets configuration for the internal Authok client. This can be
