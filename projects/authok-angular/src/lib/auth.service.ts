  ...
  implements OnDestroy
{
  ...
  constructor(
    @Inject(AuthokClientService) private authokClient: AuthokClient,
    ...
    const checkSessionOrCallback$ = (isCallback: boolean) =>
      iif(
        ...
        defer(() => this.authokClient.checkSession())
      );
  ...
  loginWithRedirect(
    ...
    return from(this.authokClient.loginWithRedirect(options));
  }
  ...
  loginWithPopup(
    ...
    return from(
      this.authokClient.loginWithPopup(options, config).then(() => {
   ...
   * [Read more about how Logout works at Authok](https://docs.authok.cn/docs/logout).
   *
   * @param options The logout options
   */
  logout(options?: LogoutOptions): void {
    const logout = this.authokClient.logout(options) || of(null);
   ...
   * the `authok` cookie, and thus will not work in browsers that block third-party
  ...
  getAccessTokenSilently(
    ...
    return of(this.authokClient).pipe(
      concatMap((client) =>
  ...
  getUser<TUser extends User>(
    ...
    return defer(() => this.authokClient.getUser<TUser>(options));
  }
  ...
  getIdTokenClaims(
    ...
    return defer(() => this.authokClient.getIdTokenClaims(options));
  }
   ...
   * responses from Authok. If the response is successful, results
   * will be valid according to their expiration times.
   *
  ...
  handleRedirectCallback(
    ...
    return defer(() =>
      this.authokClient.handleRedirectCallback<TAppState>(url)
    ).pipe(
  ...
  buildAuthorizeUrl(options?: RedirectLoginOptions): Observable<string> {
    return defer(() => this.authokClient.buildAuthorizeUrl(options));
  }
  ...
  buildLogoutUrl(options?: LogoutUrlOptions): Observable<string> {
    return of(this.authokClient.buildLogoutUrl(options));
  }
